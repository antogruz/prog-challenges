...
curl_request = "curl '{address}' -H 'Cookie: PHPSESSID={sessid}; SMFCookie89={smf}'"
import os

def get_page_with_cookie(address):
    return os.popen(curl_request.format(address=address, sessid=sessid, smf=smfcookie89)).read()

def get_file_with_cookie(address, output):
    return os.popen((curl_request + " --output {output}").format(address=address, sessid=sessid, smf=smfcookie89, output=output)).read()
